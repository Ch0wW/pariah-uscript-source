class PariahServerAdminSpectator extends MessagingSpectator
	config;
...
event Destroyed()
{
	// HACK: Seems like the PariahServerAdmin Pointer becomes corrupted
...
function String FormatMessage(PlayerReplicationInfo PRI, String Text, name Type)
{
	...
	if (PRI != None) {
		if (Type == 'Say' && PRI == PlayerReplicationInfo)
			...
			Message = PRI.RetrivePlayerName()$": "$Text;
