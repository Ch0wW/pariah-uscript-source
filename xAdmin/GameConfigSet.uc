...
var protected xMapLists			MapHandler;		// Custom map controller
...
function bool StartEdit(optional string GameType)
{
	...
	GameIndex = MapHandler.GetGameIndex(string(GameClass));
...
//		Log("CGS.NextMutators="@NextMutators);
		MapHandler.SaveGame(GameIndex);
	}

	else MapHandler.CancelChange(GameIndex);
...
function array<string> GetLists()
{
	...
	Names = MapHandler.GetMapListNames(GameIndex);
	return Names;
}

function int GetActiveList()
{
	return MapHandler.GetActiveList(GameIndex);
}
...
{
	return MapHandler.GetMapList(GameIndex, CurrentMapList);
}
		...
		for (i = 0; i<AllMaps.Length; i++)
		{
			for (j = 0; j<FoundMasks.Length; j++)
			{
				if (MaskedCompare(AllMaps[i], FoundMasks[j]))
				{
					...
					if (!bFound)
					{
						MapHandler.AddMap(GameIndex, CurrentMapList, AllMaps[i]);
		...
		for (i=0; i<CurrentMaps.Length; i++)
		{
			for (j=0; j<FoundMasks.Length; j++)
			{
				if (MaskedCompare(CurrentMaps[i], FoundMasks[j]))
				{
					...
					MapHandler.RemoveMap(GameIndex, CurrentMapList, CurrentMaps[i]);
					i--;
					break;
				}
			}
...
local int i,j;
	...
	//TODO: Change GameInfo.LoadMapList to support loading ALL maps if MapPrefix is ""
	...
	// HACK until change GameInfo.LoadMapList
	if (MapPrefix == "")
	{
		for (i = 0; i < AllGameTypes.Length; i++)
		{
			Tmp.Length = 0;
			if (AllGameTypes[i].default.MapPrefix != "")
				AllGameTypes[i].static.LoadMapList(AllGameTypes[i].default.MapPrefix, Tmp);
