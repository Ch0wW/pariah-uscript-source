...
native(190) static final function     float Atan  ( float A, float B ); //amb: changed to atan2
...
native(247) static final function     float Lerp  ( float Alpha, float A, float B, optional bool bClampRange ); //amb: added optional clamp
...
native(537) final function name GetCaller();
...
// Properties.
// amb --- simulated for debugging only! TODO: remove simulated keyword later
native final simulated function string GetPropertyText( string PropName ); 
native final simulated function bool   SetPropertyText( string PropName, string PropValue );
// --- amb
...
native static final function object LoadPackage( string PackageName ); // gam
...
final simulated function float RandRange( float Min, float Max )
{
    return Min + (Max - Min) * FRand();
}

//amb: for script timing
native(535) static final function float StopWatch
(
    optional string     Desc, 
    optional bool       bStop
);
native static final function bool IsOnConsole();   //amb: for console specific stuff
native static final function bool IsSoaking();     //gam: for making legumes soft
native static final function bool IsDebugging();   //gam: if -debugging is passed in on the cmd-line
native static final function bool IsShipping();	// rj: if this is a shipping (or demo) build
native static final function bool IsMiniEd();		//msp: is it mini-ed running
//=============================================================================
...
event LanguageChange(); //gam: sent to any object who's class is flagged as CLASS_Localized

native(197) final iterator function AllObjects(class baseClass, out Object obj); //amb
native(198) static final function PreLoad(Object obj); //amb
...
static simulated function string ReplaceSubstring( string Input, string ToReplace, coerce string Replacement )
{
	local int i;
	
	Assert( Len(ToReplace) > 0 );
	
	ToReplace = Caps(ToReplace);
	
	i = InStr( Caps(Input), ToReplace );
	
	if( (i < 0) && (Len(ToReplace) > 1) )
	{
        // Only bitch if we're replacing more than 1 char -- typically safe to suppress that warning.
        log("Could not find marker" @ ToReplace @ "in:" @ Input, 'Error');
	}
	
	while( i >= 0 )
	{
		Input = Left( Input, i ) $ Replacement $ Mid( Input, i+Len(ToReplace) );
	    i = InStr( Caps(Input), ToReplace );
	}
...
static final function int    Split  ( coerce string Src, string Divider, out array<string> Parts )
{
	local int i, p;
...
static final function bool   Divide ( coerce string Src, string Divider, out string LeftPart, out string RightPart)
{
	...
	InPos = InStr( Src, Divider );
	if( InPos<0 )
		return false;
	LeftPart = Left( Src, InPos );
	RightPart = Mid( Src, InPos + Len( Divider ) );
	return true;
}
