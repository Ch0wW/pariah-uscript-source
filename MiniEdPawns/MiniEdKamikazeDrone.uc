class MiniEdKamikazeDrone extends MiniEdAIDrone;
...
{
	return class'MiniEdPawnKamikazeDrone';
}
...
//		log(dist$" -> Checking "$victim);
		if(victim != Pawn && !victim.IsA('MiniEdDrone') && VSize(victim.Velocity) > SenseVelocity) {
...
{
	if(!Other.IsA('MiniEdDrone') && !Other.bPaused && DroneCanSee(Other) && VSize(Other.Velocity) > SenseVelocity && Other != Pawn)
	{
...
BEGIN:
	...
	MiniEdPawnKamikazeDrone(Pawn).bChasing = true;
...
//			log("   (b) new area loc = "$MyDroneArea.Location);
		...
		MiniEdPawnKamikazeDrone(Pawn).bChasing = false;
	}
