...
var()	name			FromObjectiveTag;	//This path can only be used when travelling to ObjectiveTag FROM FromObjectiveTag
...
function bool ValidFrom(GameObjective LastSquadObjective)
{
	if(FromObjectiveTag == '')
		return true;

	if(LastSquadObjective != None && LastSquadObjective.Tag == FromObjectiveTag)
		return true;
