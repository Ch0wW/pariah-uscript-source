...
function PostBeginPlay()
{
	...
	if ( (MyBaseVolume != None) && (MyBaseVolume.LocationName ~= "unspecified") )
		MyBaseVolume.LocationName = LocationPrefix@RetrivePlayerName()@LocationPostfix;
...
function Timer()
{
	if ( DefenderTeamIndex < 2 )
	{
		...
		if (Level.Game.GameStats!=None)
			Level.Game.GameStats.TeamScoreEvent(DefenderTeamIndex,Score,"game_objective_score");
	}
}
...
function string RetrivePlayerName()
{
	...
	return "Team"$DefenderTeamIndex$"Objective"$Name;
}
...
function DisableObjective(Pawn Instigator)
{
	...
	{
		Level.Game.BroadcastLocalizedMessage(class'CTFMessage', 7);
	}
	if ( bTeamControlled )
