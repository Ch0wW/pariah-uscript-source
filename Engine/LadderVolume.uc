...
simulated function bool IsAtTop(Pawn P)
{
	local Ladder L, Nearest;
	local float f, nearestdist;

	nearestdist=1000000;

	for( L = LadderList; L != None; L = L.LadderList )
	{
		f = VSize(L.Location - P.Location);
		if(f <= nearestdist)
		{
			if(Nearest == L) // the ladder list forms a cycle?
			{
				break;
			}
			nearestdist = f;
...
simulated event PawnEnteredVolume(Pawn P)
{
	...
	else if ( P != None && P.Controller != None )
	{
