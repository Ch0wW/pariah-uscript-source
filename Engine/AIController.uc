...
event NotifyRunOver(Pawn carThreat) {}
...
// MH
// Same as WeaponFire again, but rather than parameterize, give an explicit function
...
function vector AdjustToss(float TSpeed, vector Start, vector End, bool bNormalize)
{
	...
	if ( Start.Z > End.Z + 64 )
	{
		...
		Result = Pawn.SuggestFallVelocity(Dest2D,Start,TSpeed,TSpeed);
		Vel2D = result;
		Vel2D.Z = 0;
	...
	{
		Result = Pawn.SuggestFallVelocity(End,Start,TSpeed,TSpeed);
	}
	if ( bNormalize )
...
function DisplayDebug(Canvas Canvas, out float YL, out float YPos)
{
	...
	Canvas.DrawText("     Skill "$Skill$" NAVIGATION MoveTarget "$GetItemName(String(MoveTarget))$" PendingMover "$PendingMover$" MoveTimer "$MoveTimer);
	YPos += YL;
	Canvas.SetPos(4,YPos);

	Canvas.DrawText("      Destination "$Destination$" Focus "$GetItemName(string(Focus))$" Preparing Move "$bPreparingMove);
	YPos += YL;
	Canvas.SetPos(4,YPos);

	if ( InventorySpot(RouteGoal) != None && InventorySpot(RouteGoal).markedItem != None)
		Canvas.DrawText("      RouteGoal "$GetItemName(string(RouteGoal))$" Desire "$ Desireability( InventorySpot(RouteGoal).markedItem) $" RouteDist "$RouteDist);
	else
		Canvas.DrawText("      RouteGoal "$GetItemName(string(RouteGoal))$" RouteDist "$RouteDist);
	YPos += YL;
	Canvas.SetPos(4,YPos);
